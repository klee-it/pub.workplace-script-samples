#
## basic information:
## |__ This script will check and detect if long paths are enabled
#
...
try
{
    ...
    {
        Write-Logging -Value "# Registry Settings..."
    
        foreach ($RegSetting in $DeviceConfig.RegistrySettings.list)
        {
            if ($RegSetting.Action -eq "add")
            {
                $RegSettingStatus = Get-ItemProperty -Path "$($RegSetting.Path)" -Name "$($RegSetting.Name)" -ErrorAction 'SilentlyContinue' | Where-Object {$_.($RegSetting.Name) -eq "$($RegSetting.Value)"};

                if (-Not ($RegSettingStatus) )
                {
                    Write-Logging -Value "|__ Registry setting [$($RegSetting.Path)\\$($RegSetting.Name)] is not set to value $($RegSetting.Value)"
                    $exit_code = 1
                }
            }

            if ($RegSetting.Action -eq "remove")
            {
                $RegSettingStatus = Get-ItemProperty -Path "$($RegSetting.Path)" -Name "$($RegSetting.Name)" -ErrorAction 'SilentlyContinue';

                if ($RegSettingStatus)
                {
                    Write-Logging -Value "|__ Registry setting [$($RegSetting.Path)\\$($RegSetting.Name)] is not removed"
                    $exit_code = 1
                }
            }
        }
