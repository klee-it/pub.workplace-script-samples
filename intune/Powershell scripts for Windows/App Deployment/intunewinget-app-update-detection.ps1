...
try
{
    ...
    Get-Variable -Name 'Splat' -ErrorAction 'SilentlyContinue' | Remove-Variable -Force
    Write-Logging -Value "[SR] Number of local apps: $($LocalAppInfo.ConsoleOutput.Content.Count)"
    ...
    foreach ($Id in $Ids)
    {
        ...
        {
            Write-Logging -Value "[SR] Local installed application name: '$($WinGetAppInfo.Name)' with version: '$($WinGetAppInfo.Version)'"

            if ( [String]::IsNullOrEmpty($WinGetAppInfo.Available) -eq $false )
            {
                Write-Logging -Value "[SR] Update for $($WinGetAppInfo.Name) to v$($WinGetAppInfo.Available) required - client has older version $($WinGetAppInfo.Version)"
                $exit_code = 1
            }
            else
            {
                Write-Logging -Value "[SR] Update not required - client has latest version $($WinGetAppInfo.Version)"
            }
        ...
        { 
            Write-Logging -Value "[SR] Too many applications found: $($WinGetAppInfo.Name -join ' / ')"
        }
