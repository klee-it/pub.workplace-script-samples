...
try
{
    ...
    Write-Logging -Value '[SR] Script: intunewin-app-installation-detection'
    ...
    { 
        Write-Logging -Value "[SR] Specified application: $($AppName) is either invalid or application is not installed"
        $exit_code = 1
    }
    elseif ( ($LocalAppInfo | Measure-Object).Count -eq 1 )
    {
        Write-Logging -Value "[SR] Specified application: '$($LocalAppInfo.DisplayName)' with version: '$($LocalAppInfo.DisplayVersion)' is already installed"
        $exit_code = 0
    }
    else
    {
