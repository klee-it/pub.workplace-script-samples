...
try
{
    ...
    Write-Logging -Value '[SR] Script: intunewinget-app-installation-detection' -StdOut 'None'
    Write-Logging -Value "[SR] App Name: $($AppName)" -StdOut 'None'
    Write-Logging -Value "[SR] IDs: $($Ids -join ', ')" -StdOut 'None'

    # get Windows principal details
    Write-Logging -Value '[SR] Get local system details...' -StdOut 'None'
    ...
    foreach ($Id in $Ids)
    {
        Write-Logging -Value "[SR] ID: $($Id)" -StdOut 'None'
        ...
        {
            Write-Logging -Value "[SR] Specified application: $($Id) is not installed" -StdOut 'None'
            Write-Logging -Value '[SR] Deployment applicable' -StdOut 'None'
            $IsApplicable = $true
        }
        elseif ( $outputInfo.ConsoleOutput.Content.Count -eq 1 )
        {
            Write-Logging -Value "[SR] Specified application: '$($outputInfo.ConsoleOutput.Content.Name)' with version: '$($outputInfo.ConsoleOutput.Content.Version)' is already installed" -StdOut 'None'
            Write-Logging -Value '[SR] Skip deployment' -StdOut 'None'
        }
        else
        {
            Write-Logging -Value "[SR] Too many applications found: $($outputInfo.ConsoleOutput.Content.Name -join ' / ')" -StdOut 'None'
            Write-Logging -Value '[SR] Skip deployment' -StdOut 'None'
        }
    ...
    Write-Logging -Value "[SR] Exit code for Intune: IsApplicable: $($IsApplicable)" -StdOut 'None'
