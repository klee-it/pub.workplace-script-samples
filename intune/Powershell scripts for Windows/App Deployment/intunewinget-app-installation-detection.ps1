...
try
{
    ...
    Write-Logging -Value '[SR] Script: intunewinget-app-installation-detection'
    ...
    foreach ($Id in $Ids)
    {
        ...
        {
            Write-Logging -Value "[SR] Specified application: $($Id) is either invalid or application is not installed"
            $exit_code = 1
        }
        elseif ( $outputInfo.ConsoleOutput.Content.Count -eq 1 )
        {
            Write-Logging -Value "[SR] Specified application: '$($outputInfo.ConsoleOutput.Content.Name)' with version: '$($outputInfo.ConsoleOutput.Content.Version)' is already installed"
        }
        else
        {
